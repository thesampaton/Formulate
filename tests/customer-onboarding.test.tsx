...
describe("customer onboarding", () => {
  it("rechecks only the changed address's postcode and preserves both bindings", async () => {
    ...
    await user.selectOptions(screen.getByLabelText("Billing address country"), "US");
    ...
    await user.selectOptions(screen.getByLabelText("Delivery address country"), "US");
    await waitFor(() => expect(delivery).toHaveAttribute("aria-invalid", "true"));
    await user.selectOptions(screen.getByLabelText("Billing address country"), "AU");
  ...
  it("suspends an invalid manual draft, derives fresh delivery, and restores/rechecks the draft", async () => {
    ...
    expect(screen.getByLabelText("Delivery address country")).toHaveValue("US");
