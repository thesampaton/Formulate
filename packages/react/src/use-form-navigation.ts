...
export type CorrectionDestination<Values extends FieldValues, Page extends string> = {
  /** Explicit RHF path of the editor to focus. List destinations in correction priority order. */
...
export function useFormNavigation<Values extends FieldValues, Page extends string>({
  ...
  function goToField(name: FieldPath<Values>): boolean {
    ...
    destination.reveal?.();
    goTo(destination.page, () => form.setFocus(destination.name));
    return true;
  }
  ...
  function correct(errors: FieldErrors<Values>): boolean {
    const destination = destinations.find(({ name }) => get(errors, name));
    if (!destination) return false;
    return goToField(destination.name);
  }
