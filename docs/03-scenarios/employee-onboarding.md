...
library page EmploymentSetup
  ...
  layout default: shadcn FieldGroup
  present details
...
| Layout | shadcn FieldGroup default | The same shadcn FieldGroup default |
...
2. Leave each required field empty in turn. Continue is blocked by the same local rule in either host and focuses that host's correct editor. A valid setup can advance in onboarding while Equipment is still incomplete; it cannot submit the whole form.
...
5. Assert each exact payload root. Tests also replace the page layout without changing bindings or rules; final validation and payload remain identical for that host.
