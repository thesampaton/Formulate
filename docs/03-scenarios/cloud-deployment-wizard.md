...
The [authoring correction](../05-06-rendering-and-workflow.md#course-corrections-retained) keeps this same sequence and shares the dependent-choice integration with Infrastructure. Primary and Recovery bound uses now supply presentation identity, local choice views and the Targets action/correction scope. The definition installs its choice runtime from services, and Form checks its live evidence revision at action boundaries; hosts no longer repeat schema parsing or wire the revision prop.
