...
- [Sample data](../../examples/react/src/data/example-data.ts) supplies the sample loader and test prefills. Consumers clone the values before editing them.
...
A per-page `layout` replaces the inherited layout and `layout={null}` removes it, using the existing core layout contract. These shadcn wrappers do not change Page or introduce another form runtime. The [example page](../../examples/react/src/guides/multi-page.ts) shows the page rules and correction path through short source excerpts.
...
The demo wrapper owns sample loading. The resize demonstration similarly owns its slider outside ProfileForm. The example page keeps complete application files available on demand after its focused walkthrough.
