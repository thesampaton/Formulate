...
- [x] **Connect dependent choices to reusable definitions.** Fields accept `choices`; definitions recursively expose `bindChoices`. A reused section carries its internal input and membership wiring. The [runtime hook](../packages/react/src/use-choice-form.ts) uses `useFormulate`; RHF still owns editing values.
