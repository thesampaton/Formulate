...
**Status: implemented and revised to use existing primitives first, September 2026.** Cloud Deployment and Infrastructure consume definition-owned dependent choices from `@formulate/react`. React Activity owns inactive page lifecycles, transitions own action pending state, and AbortSignal identifies obsolete requests. Resume the remaining [Parts 5–6 decisions](05-06-rendering-and-workflow.md#what-still-needs-doing), starting with bound section scopes and correction destinations. The [design principles](02-design-principles.md) remain the acceptance standard; this bounded extraction does not establish a general workflow engine or an overall productivity claim.
...
- [x] **Connect dependent choices to reusable definitions.** Fields accept `choices`; definitions recursively expose `bindChoices`. A reused section carries its internal input and membership wiring. The [runtime hook](../packages/react/src/choices/use-choice-form.ts) uses `useFormulate`; RHF still owns editing values.
...
Applications still own branch destinations, correction policy, durable IDs, draft storage/compatibility, plan acceptance and execution. RHF owns editing values, array operations and reset. Layouts/actions/navigation retain their existing registry items. The Employment adapter remains local.
...
The revised implementation grows by 99 lines against the pre-extraction baseline. The first extraction counted 824 lines across its narrower scope; across that same scope this revision uses 816. Including the additional lifecycle/action/control changes leaves the second pass one line larger overall than the first. Using existing primitives is a maintenance decision, not a size-reduction claim. The definition integration removes independently maintained relationships when a reusable unit changes:
...
- A choice form explicitly installs `useChoiceForm`, a stable `fields` binder and Form's evidence revision getter. Plain `Definition.useForm()` performs schema validation only.
- Custom presentation still reads a use ID plus its rule to supply options, feedback and retry. Definition `Fields` does not automatically render a dependent picker. A changed field name may require changes to those presentation lookups.
- Repeated uses share one `resourceBindings` map between behavior and rendering, but callers still provide it at both boundaries. Bound section scopes and correction destinations are the next candidate; no new page API was introduced.
- Conditional applicability is expressed by selecting bound uses and by application schemas. General completion, readiness, applicability and cross-page relationships remain unresolved Parts 5–6 work.
- Stable callbacks, complete dependency defaults and explicit request keys are required. There is no general dependency engine, caching or deduplication. Services that ignore abort are safe against obsolete publication, but arbitrary async schema results can still update RHF errors after cancelled actions.

## Delivery and verification

`defineChoice` and `useChoiceForm`, plus the choice contract types, are package exports. `bindChoices` belongs to form/section definitions. Request/store constructors are private modules. `@formulate/core` includes all four new source modules in its manifest; package builds emit the same implementation and declarations. No new package, UI registry item or external dependency was added. Domain declarations and picker presentation remain editable application source.
