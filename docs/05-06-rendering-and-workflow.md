...
| Structured editors and compound pickers | `useCompoundFieldBinding` connects one structured value to a primary trigger and logical popup blur boundary. Optional shadcn date-range and multiple-selection adapters cover objects, arrays, partial values, reset, disabled state, and portal lifecycle under Activity. Nested issues display on the root editor. | [Control guide](../packages/react/docs/custom-controls.md#structured-values-and-compound-pickers), [tests](../tests/structured-editing.test.tsx). |
| Rendering slots, headings and portals | Field chrome slots, container body slots, nested/explicit heading levels, and a portal destination provider. The picker shim supports a scoped theme without patching installed shadcn UI. Examples use a single `globals.css` with CSS-variable tokens and Tailwind spacing/type defaults. | [Structured picker scenario](03-scenarios/structured-editing.md). |
