...
| Shared choice helpers still required each host to reconstruct local dependency wiring. | **Extracted:** [package choice integration](../packages/react/src/choices/use-choice-form.ts) and recursive definition binding now carry local input/membership rules. Multiple loaders, generic choices and transformed outputs are covered. Services, branches, drafts and plans remain application-owned. | Repeated presentation/scope bindings or another concrete dependency shape demonstrates a missing shared responsibility. |
